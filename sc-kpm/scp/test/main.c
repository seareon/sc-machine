...
sc_result printElem(sc_memory_context *context, sc_addr element)
{
    ...
    while (SC_TRUE == sc_iterator3_next(it))
    {
        ...
        if (SC_RESULT_OK == sc_helper_get_system_identifier_link(context, addr3, &idtf))
        {
            sc_stream *stream;
            sc_uint32 length = 0, read_length = 0;
            sc_char *data;
        ...
        if (SC_RESULT_OK == sc_helper_get_system_identifier_link(context, addr2, &idtf))
        {
            sc_stream *stream;
            sc_uint32 length = 0, read_length = 0;
            sc_char *data;
...
void test_math(sc_memory_context *context, const char *name1, const char *name2)
{
    scp_operand num1, num2, num3;
    MAKE_DEFAULT_OPERAND_FIXED(num1);
    MAKE_DEFAULT_OPERAND_FIXED(num2);
    MAKE_DEFAULT_OPERAND_FIXED(num3);

    sc_helper_resolve_system_identifier(context, name1, &num1.addr);
    sc_helper_resolve_system_identifier(context, name2, &num2.addr);

    printNl(context, &num1);
    printNl(context, &num2);
    //contAdd(context, &num3, &num1, &num2);
    //printNl(context, &num3);
}
